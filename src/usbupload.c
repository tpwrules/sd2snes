...
volatile unsigned char send_buffer[512];	
...
//requestet by usb host
	...
	if(usb_handler_flags & (USB_SEND_ROM|USB_SEND_RAM) ){
		...
		//info pkt stage
          if(send_stage==0){
            ...
            //	printf("call CDC_block_send stage=%d size=%d\n",stage,sizeof(send_buffer));
            while(CDC_block_send((unsigned char*)send_buffer, sizeof(send_buffer)) == -1) {}
            ...
            if(usb_handler_flags & USB_SEND_ROM){
              ...
              sram_readblock((unsigned char*)send_buffer,  SRAM_ROM_ADDR+bytes_offset, 0x200);
					
            } else if(usb_handler_flags & USB_SEND_RAM){
					
              sram_readblock((unsigned char*)send_buffer, SRAM_SAVE_ADDR+bytes_offset, 0x200);
...
void usb_send(uint8_t usb_flags){
	...
	printf("usb_filesize %d\n", usb_filesize);
		...
		memset((unsigned char*)send_buffer, 0, 512);
		
		send_buffer[0]='F';
		send_buffer[1]='S';
		send_buffer[3] = (uint8_t) ((( usb_filesize) / 0x200) >> 8); //length
		send_buffer[4] = (uint8_t) (usb_filesize / 0x200); //length
		...
		else if(usb_flags & USB_SEND_RAM){
			sending=1;
			//usb_rec_request=USB_SEND_RAM;
			//usb_handler_flags = USB_LOCK;
		}
	...
	offset+=length;
       ...
       if(offset==0x200){ //512-64 process 512er buffer
		   ...
		   if(datablocks==0){ //check for command pkt if no datablocks are incoming
			   ...
			   if(buffer[0]=='U' && buffer[1]=='S' && buffer[2]=='B' && buffer[3]=='R'){
				   //read mode
				...
				//force unlock
				if(buffer[0]=='U' && buffer[1]=='S' && buffer[2]=='B' && buffer[3]=='U'){
				...
				//force lock
				if(buffer[0]=='U' && buffer[1]=='S' && buffer[2]=='B' && buffer[3]=='L'){

					usb_handler_flags = USB_LOCK;
					printf("usb: force lock\n");
				}else
				//set time
				if(buffer[0]=='U' && buffer[1]=='S' && buffer[2]=='B' && buffer[3]=='T'){
				...
				//mapper command
				if(buffer[0]=='U' && buffer[1]=='S' && buffer[2]=='B' && buffer[3]=='B'){
