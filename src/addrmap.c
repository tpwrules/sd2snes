...
void addrmap_mapper(uint8_t val) {
  ...
  if (val == 0) {
      ...
      AM(2*8+0, 0x08); AM2(2*8+1, 0x0000); AM(2*8+3, 0x00); AM2(2*8+4, 0x8000); AM2(2*8+6, rommask >> 8);
      // 80-BF:8000-FFFF, C0-FF:0000-FFFF -> 00:0000
      AM(3*8+0, 0x08); AM2(3*8+1, 0x8000); AM(3*8+3, 0x00); AM2(3*8+4, 0x8000); AM2(3*8+6, rommask >> 8);
  }
  else if (val == 1) {
      // lorom
      ...
      if (rammask > 0) {
          ...
          if (rommask >= 0x400000 || rammask >= 0x10000) {
            AM(0*8+0, 0x39); AM2(0*8+1, 0x7000); AM(0*8+3, 0xE0); AM2(0*8+4, 0x7080); AM2(0*8+6, rammask >> 8);
          }
          else {
            AM(0*8+0, 0x38); AM2(0*8+1, 0x7000); AM(0*8+3, 0xE0); AM2(0*8+4, 0x7000); AM2(0*8+6, rammask >> 8);
          }
      }
      ...
      if (rommask >= 0x400000) {
        ...
        // first 4 MB -> $400000
        AM(2*8+0, 0x09); AM2(2*8+1, 0x0080); AM(2*8+3, 0x40); AM2(2*8+4, 0x8080); AM2(2*8+6, 0x3FFF);
        // remaining -> $000000
        AM(3*8+0, 0x09); AM2(3*8+1, 0x8080); AM(3*8+3, 0x00); AM2(3*8+4, 0x8080); AM2(3*8+6, (rommask-0x400000) >> 8);
      }
      else {
        AM(2*8+0, 0x09); AM2(2*8+1, 0x0080); AM(2*8+3, 0x00); AM2(2*8+4, 0x0080); AM2(2*8+6, rommask >> 8);
      }
  }
  else if (val == 2) {
      // exthirom
      ...
      AM(0*8+0, 0x20); AM2(0*8+1, 0xF000); AM(0*8+3, 0x00); AM2(0*8+4, 0xF000); AM2(0*8+6, 0xFFFF);
      // * -> C0-EF:0000-FFFF
      AM(1*8+0, 0x08); AM2(1*8+1, 0x0000); AM(1*8+3, 0xC0); AM2(1*8+4, 0x0000); AM2(1*8+6, 0x3FFF);
  }
